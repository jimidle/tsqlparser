...
,a.returner
,a.catalog_requester
,convert(bit, case when a1.mid is not null then 1 else 0 end) as ecatalog_requester
,a.store_requester
,a.siebel_contact
,a.siebel_positive
,a.siebel_negative
...
,convert(bit, isnull(z.nt_store_requester,0)) as nt_store_requester_tmp
,convert(bit, isnull(z.nw_store_requester,0)) as nw_store_requester_tmp
,convert(bit, case when ro.mid is not null then 1 else 0 end) as retail_only
,convert(bit, case when rm.mid is not null then 1 else 0 end) as returned_mail
...
left join agg.dbo.nt_nw_store_request_rollup z on a.mid = z.mid
left join agg.dbo.ecatalog_request_rollup a1 on a.mid = a1.mid
left join agg.dbo.retail_only ro on a.mid=ro.mid
left join agg.dbo.employee_in_hh_temp ehh on a.mid = ehh.mid
left join agg.dbo.buyer_level_temp bl on a.mid = bl.mid
left join agg.dbo.email_country_agg ec on a.mid=ec.mid
left join agg.dbo.return_mail_rollup rm on a.mid=rm.mid
